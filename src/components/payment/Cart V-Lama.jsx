...
const Cart = () => {
  ...
  const handleMoveAssets = async (assetDetails) => {
    try {
      ...
      // Mendefinisikan fungsi untuk menghapus satu aset dan menangani kesalahan
      const deleteAsset = async (asset) => {
        try {
          ...
          // Mencoba menghapus aset dari API
          const url = `http://localhost:3000/api/assets/${asset.assetId}`;
          await axios.delete(url);

          console.log(`Aset dengan ID ${asset.assetId} telah dihapus.`);

          // Notifikasi keberhasilan
          localStorage.setItem(
            `deleteStatus_${asset.assetId}`,
            `Aset dengan ID ${asset.assetId} telah berhasil dihapus.`
          );
        } catch (error) {
          console.error(
            `Kesalahan saat menghapus aset dengan assetId ${asset.assetId}:`,
            error
          );
          ...
          // Opsional: Implementasikan logika retry (upaya terbatas)
          let retryAttempts = 3; // Jumlah upaya pengulangan
          while (retryAttempts > 0) {
            try {
              // Mencoba menghapus dokumen lagi
              await deleteDoc(asset.docId);
              await axios.delete(asset.docId);
              console.log(
                `Penghapusan aset ID ${asset.assetId} berhasil pada percobaan ulang.`
              );
              break; // Keluar dari loop jika berhasil
            } catch (retryError) {
              console.error(
                `Pengulangan gagal untuk aset ID ${asset.assetId}:`,
                retryError
              );
              retryAttempts--;
              if (retryAttempts === 0) {
                console.error(
                  `Semua upaya pengulangan gagal untuk aset ID ${asset.assetId}.`
                );
              }
            }
          }
      ...
      // Menggunakan Promise.all untuk menangani penghapusan untuk setiap aset
