...
function ManageAsset3D() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const q = query(collection(db, 'assetImage3D'), where('userId', '==', user.uid));
  ...
  const handleDelete = async (id) => {
    const confirmDelete = window.confirm("Are you sure you want to delete this asset 3D?");
    console.log(id)
    if (confirmDelete) {
      try {
        ...
        await deleteDoc(doc(db, 'assetImage3D', id)); // Menghapus dokumen dari Firestore
          ...
          {alertError && (
            <div
              ...
              <div className="flex items-center justify-center space-x-2">
                ...
                <span>Gagal menghapus asset 3D silahkan coba lagi</span>
              </div>
            </div>
          )}
