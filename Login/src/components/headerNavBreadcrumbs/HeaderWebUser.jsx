...
import Datasetpng from "../../assets/icon/iconSidebar/dataset.png";
...
function HeaderSidebar() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const querySnapshot = await getDocs(collection(db, "cartPopUp")); // Ambil data dari koleksi
      ...
      <section className="navbar h-28 fixed z-40 top-0 left-0 pt-0 text-neutral-10  font-poppins font-semibold dark:text-primary-100 bg-primary-100 dark:bg-neutral-20 gap-2">
        ...
        <div className="gap-14 sm:gap-1 md:gap-8 lg:gap-8 xl:gap-2 2xl:gap-10 flex justify-center items-center ">
          ...
          <Link
            onClick={togglePopup}
            to=""
            className="popup-cart dropdown dropdown-end w-[20px] sm:w-[45px] md:w-[28px] lg:w-[28px] xl:w-[28px] 2xl:w-[28px] h-[20px]  sm:h-[28px] md:h-[28px] lg:h-[28px] xl:h-[28px] 2xl:h[28px] -ml-[30px]  sm:ml-1 md:ml-1 lg:ml-0 xl:ml-0 2xl:ml-2 gap-2  text-[8px] sm:text-[10px] md:text-[10px] lg:text-[10px] xl:text-[10px 2xl:text-[10px]">
            <img src={IconCart} alt="icon cart" className="w-[24px] h-[24px]" />
            <label className="">Cart</label>
          </Link>
          {isPopupVisible && (
              <div className="space-y-4 rounded-lg overflow-y-auto absolute flex-col top-20 right-8 w-[350px] h-[400px] bg-white shadow-lg rounded-lg p-4 z-10">
                <p className="mb-30 text-[#000000] font-semibold">Isi Keranjang</p>
                <div className="flex-col space-y-4 w-full h-[430px] rounded-lg overflow-y-auto h-3">
                    ...
                    <div key={index}>
                      ...
                      <div className="flex bg-blue-100 h-20 rounded-[10px]">
                        <button className="mt-0 w-20 h-20 bg-[#F2F2F2] bg-contain rounded-[10px]">
                          <img src={item.datasetImage || item.uploadUrlImage} alt="Datasetzip" className='w-full h-full object-cover rounded-[10px]' />
                        </button>
                        <div className="ml-5 content-center text-left">
                          ...
                          <div className='flex'>
                            <div>
                              <button className='text-sm font-medium text-[#141414] hover:underline hover:text-red-600'>Remove</button>
                            </div>
                            <div>
                              <p className='ml-3 mt-1 text-[#141414] justify-self-end text-sm'>Rp.{item.price}</p>
                            </div>
                          </div>
                        </div>
                      </div>
                ...
                <div className='grid'>
                  <p className="justify-self-end text-sm text-[#000000] font-semibold">Subtotal ( {itemCount} Item ): Rp.125.000,00 </p>
                </div>
                ...
                <div className=''>
                  <div className='flex items-center justify-center'>
                    <button
                      onClick={togglePopup}
                      className="text-center mt-1 bg-[#5487F8] text-white px-4 py-2 rounded hover:bg-[#2563EB]"
                    >
                      Checkout
                    </button>
                  </div>
                  <div className='flex items-center justify-center mt-2'>
                    <button
                      onClick={togglePopup}
                      className="mt-0 text-[#2563EB] px-0 py-0 hover:underline"
                    >
                      Lihat Keranjang
                    </button>
                  </div>
