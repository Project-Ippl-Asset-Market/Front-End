  ...
  const fetchAssets = async (selectedSubCategory) => {
    ...
    try {
      const promises = collectionsFetch.map((collectionName) => {
        let q;
        // Jika ada kategori dan subkategori yang dipilih, gunakan query where
        if (selectedSubCategory) {
          // console.log(selectedSubCategory)
          // Jika hanya kategori yang dipilih, tanpa subkategori
          q = query(
            collection(db, collectionName),
            where("category", "==", selectedSubCategory)
          );
        } else {
          // Jika tidak ada filter kategori, ambil semua dokumen
