...
function AddNewImage() {
  ...
  const handleImageUpload = async () => {
    ...
    if (!imageName || !category || !description || !price) {
      alert("Please fill all the required fields.");
      return;
    }

    if (!user) {
      alert("You must be logged in to upload a image.");
      return;
    }

    setLoading(true); // Set loading to true
    const storageRef = ref(storage, `images-asset/${imageFile.name}`);
    ...
    try {
      ...
      await addDoc(collection(db, "assetImages"), {
        imageName,
        category,
        description,
        price,
        uploadUrlImage: downloadURL,
