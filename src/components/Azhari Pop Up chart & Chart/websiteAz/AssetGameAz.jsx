...
import { getDocs, collection, doc, addDoc, getDoc, updateDoc } from 'firebase/firestore';
    ...
    <div className="fixed inset-0 bg-black bg-opacity-60 flex justify-center items-center z-50">
      <div className="bg-[#5D5F5F] dark:bg-[#171717] p-8 sm:p-12 rounded-lg relative w-[320px] h-[570px] sm:w-[580px] sm:h-[660px] lg:w-[925px] lg:h-[700px] font-poppins">
        ...
        <div className="relative flex">
          <div className="-ml-4">
            <img 
            src={asset.datasetImage || DefaultPreview} 
            alt="Asset" className="w-[170px] h-[200px] sm:w-[255px] sm:h-[300px]  lg:w-[450px] lg:h-[530px] object-cover mb-2 "
        ...
        <div className="block lg:hidden flex-col sm:pt-14 items-center gap-5 text-white mt-4 sm:mt-0">
          ...
          <button 
            onClick={() => handleAddCart(asset.assetId)}
            className="flex w-[260px] h-[50px] sm:w-[300px] items-center bg-[#575859] rounded-md">
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const querySnapshot = await getDocs(collection(db, 'assetDatasets')); // Ganti 'nama_koleksi' dengan nama koleksi di Firestore
        const items = [];
        ...
        // Susun data ke dalam format assets
          items.push({
            ...
            datasetName: data.datasetName,       // Nama dataset dari Firestore
            description: data.description,      // Deskripsi dataset dari Firestore
            price: `Rp. ${data.price}`,         // Harga dataset dari Firestore
            datasetImage: data.datasetImage,   // URL gambar dataset dari Firestore
          });
  ...
  return (
    <div className="bg-white dark:bg-[#212121]">
      ...
      <main className="bg-white dark:bg-[#212121] mt-36">
        <h1 className="bg-white dark:bg-[#212121] text-lg sm:text-xl font-semibold mt-6 mb-4 ml-4 sm:ml-16 font-roboto text-[#000000] dark:text-[#FFFFFF]">Asset Game</h1>
          ...
          {assets.map((asset, index) => (
            <div 
              ...
              <div className="flex justify-center">
                <img src={asset.datasetImage || DefaultPreview} 
                alt="Product" className="w-full h-48 object-cover mb-2" 
                onError={(e) => { e.target.src = DefaultPreview; }}
                />
              </div>
