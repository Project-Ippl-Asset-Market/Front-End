...
import { ref, uploadBytes, getDownloadURL } from "firebase/storage"; // Tambahkan import untuk Storage
import { auth, db, storage } from "../../firebase/firebaseConfig"; // Pastikan storage sudah dikonfigurasi
...
function AddNewAdmin() {
  const [admin, setAdmin] = useState({
    ...
    profileImage: null, // Tambahkan untuk menyimpan file image
  });
  ...
  const handleChange = (e) => {
    if (e.target.name === "profileImage") {
      setAdmin({
        ...admin,
        profileImage: e.target.files[0], // Ambil file gambar dari input
      });
    } else {
      setAdmin({
        ...admin,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // 2. Upload profile image to Firebase Storage in /images-admin folder
      let profileImageUrl = "";
      if (admin.profileImage) {
        const imageRef = ref(
          storage,
          `images-admin/${user.uid}/profileImage.jpg`
        ); // Mengubah path ke /images-admin
        await uploadBytes(imageRef, admin.profileImage);
        profileImageUrl = await getDownloadURL(imageRef); // Dapatkan URL gambar yang diupload
      }

      // 3. Save user details along with role and profile image URL to Firestore
      await addDoc(collection(db, "admins"), {
        ...
        profileImageUrl: profileImageUrl, // Simpan URL gambar profile
      });
