...
export const createTransactionController = async (req, res) => {
  try {
    ...
    const itemDetails = assets.map((asset) => ({
      id: asset.assetId,
      price: Number(asset.price) + Number(asset.price) * 0.1, // Convert price to number
      ...
      subtotal: Number(asset.price) + Number(asset.price) * 0.1, // Calculate subtotal ensuring it's a number
    }));
    ...
    const formattedGrossAmount = totalCalculated;
